...
func (h *FixturesHandler) Live(w http.ResponseWriter, r *http.Request) {
	...
	if h.db != nil {
		fixtures := h.queryFixtures(ctx, `
			...
			WHERE f.is_live = true ORDER BY f.starts_at DESC LIMIT 20`)
		if len(fixtures) > 0 {
	...
	writeJSON(w, fromProvider(pf, 20, true))
}

// ByDate returns fixtures for ?date=YYYY-MM-DD (defaults to today)
func (h *FixturesHandler) ByDate(w http.ResponseWriter, r *http.Request) {
	...
	if h.db != nil {
		fixtures := h.queryFixturesWithArg(ctx, `
			...
			ORDER BY f.starts_at ASC LIMIT 50`, date.Format("2006-01-02"))
		if len(fixtures) > 0 {
			writeJSON(w, fixtures)
			return
		}
	...
	writeJSON(w, fromProvider(pf, 50, false))
}

// Today is a convenience alias for ByDate with today's date
...
func (h *FixturesHandler) queryFixtures(ctx context.Context, sql string) []FixtureResponse {
...
func fromProvider(pf []provider.ProviderFixture, limit int, forceIsLive bool) []FixtureResponse {
	var out []FixtureResponse
	for i, f := range pf {
		if i >= limit {
			break
