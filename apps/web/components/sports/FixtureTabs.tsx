...
function LeagueGroup({ league, fixtures, leagueLogoUrl, defaultExpanded = false }: { league: string; fixtures: Fixture[]; leagueLogoUrl?: string; defaultExpanded?: boolean }) {
...
export function FixtureTabs({ sport = 'football', timeRange = 6, leagueId, activeTab = 'prematch' }: FixtureTabsProps) {
  ...
  const fetchFixtures = useCallback(async () => {
    setLoading(true);
    setVisibleCount(PAGE_SIZE); // reset pagination on new fetch
    try {
  ...
  return (
    <div>
      {loading ? (
        ...
        <div>
          {Object.entries(grouped).map(([league, { fixtures, logoUrl }], index) => (
            <LeagueGroup key={league} league={league} fixtures={fixtures} leagueLogoUrl={logoUrl} defaultExpanded={index === 0} />
          ))}
